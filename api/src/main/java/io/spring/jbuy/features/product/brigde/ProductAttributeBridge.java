...
public class ProductAttributeBridge implements PropertyBridge<List> {
    ...
    private static final Set<String> excludeAttributeList = new HashSet<>(Set.of(
    ...
    public void write(DocumentElement target, List bridgedElement, PropertyBridgeWriteContext context) {
        ...
        for (ProductAttribute productAttribute : productAttributeList) {
            ...
            if (attributeName.contains("Weight")) {
                if (categorySet.contains("Laptop")) {
                ...
                } else if (categorySet.contains("Monitor") || categorySet.contains("Desktop")) {
                    if (size >= 50) {
                ...
                } else if (categorySet.contains("Laptop")) {
                    if (size >= 17) {
                ...
                for (String token : values) {
                    token = token.trim();
                    target.addValue(this.productAttributeField,
                                    token.toLowerCase() + "|" + attributeType + "__" + attributeName + "__" + token);
                }
            } else if (value.contains(",")) {
                String[] values = value.split(", ");
                for (String token : values) {
                ...
                String targetValue = value.toLowerCase() + "|" + attributeType + "__" + attributeName + "__" + value;
